...
>   （自動回流工具尚未就緒 —— 見
>   [parallel-ai-agents#39](https://github.com/PsychQuant/parallel-ai-agents/issues/39)。
>   本機寫好的 lens 目前請手動改 `plugins/pai-lenses/lenses/<profile>.csv` 並 bump 兩處版本。）
>
> **安裝**（`pai-lenses` 仍是獨立 plugin，由 `parallel-ai-agents` 的 marketplace 提供）：
>
> ```bash
> /plugin marketplace add PsychQuant/parallel-ai-agents
> /plugin install parallel-ai-agents@parallel-ai-agents
> /plugin install pai-lenses@parallel-ai-agents
> ```
>
...
> 併回之後兩層在同一棵樹上，未來的自動回流工具才有辦法判定目標層並在一個 PR 裡完成。
>
